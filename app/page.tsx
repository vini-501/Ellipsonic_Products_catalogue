...
export default function Page() {
  ...
  return (
    <main>
      ...
      <section className="suite" id="suite"><div className="section-heading"><div><p className="kicker"><span /> THE PRODUCT SUITE</p><h2>Eight products.<br /><em>Eight clear advantages.</em></h2></div><p>Explore the products below to understand what they do, who they are for, and where they can take your organization.</p></div><div className="domain-filter" aria-label="Filter products by domain"><span className="filter-label">FILTER BY DOMAIN</span><div className="filter-options">{domainOptions.map((domain) => <button key={domain.id} className={`filter-chip ${activeDomain === domain.id ? 'is-active' : ''}`} aria-pressed={activeDomain === domain.id} onClick={() => setActiveDomain(domain.id)}>{domain.label}</button>)}</div><span className="filter-count">Showing {visibleProducts.length} of {products.length}</span></div><div className="product-list">{visibleProducts.map((product) => <article className={`product product-${product.color}`} key={product.id}><div className="product-top"><div className="product-index">{product.number} <span>/ 08</span></div><div className="product-category">{product.category}</div></div><div className="product-grid"><div className="product-info"><p className="product-eyebrow">{product.eyebrow}</p><h3>{product.title}</h3><p className="product-description">{product.description}</p><div className="feature-list">{product.features.map((feature) => <span key={feature}><i>✓</i>{feature}</span>)}</div><div className="audience"><small>BEST FOR</small><div>{product.audience.map((a) => <span key={a}>{a}</span>)}</div></div><div className="product-actions"><a className="button button-dark" href={product.url ?? `#${product.id}-link`} target={product.url ? '_blank' : undefined} rel={product.url ? 'noopener noreferrer' : undefined}>View product <ArrowIcon /></a><button className="button button-light" onClick={() => setDemoProduct(product.id)}>Book a demo <ArrowIcon /></button></div></div><ProductMockup product={product} /></div></article>)}</div></section>
